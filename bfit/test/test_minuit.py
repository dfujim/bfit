...
def test_error_named_assign():
    ...
    assert_equal(m.errors['e'], 5, 'minuit errors name named assignment')
...
def test_limit_named_assign():
    ...
    assert_equal(m.limits['e'][0], 0, 'minuit limits name named assignment')
    assert_equal(m.limits['e'][1], 5, 'minuit limits name named assignment')
...
def test_fix_named_assign():
    ...
    assert_equal(m.fixed['e'], True, 'minuit fixed name named assignment')
    
    fixed_other = [m.fixed[k] for k in 'abcd']
    assert_equal(any(fixed_other), False, 'minuit fixed name named assignment, other')
